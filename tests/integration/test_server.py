...
class TestMakeToolFunctionSafeSecurity:
    ...
    def test_rejects_code_injection_in_parameter_names(self):
        ...
        malicious_schemas = [
            {
                "type": "object",
                "properties": {"__import__('os').system('ls')": {"type": "string"}},
                "required": [],
            },
            {
                "type": "object",
                "properties": {"param'; exec('print(1)'); x='": {"type": "string"}},
                "required": [],
            },
        ]
...
class TestGetPythonType:
    ...
    def test_maps_string_to_str(self):
        ...
        assert _get_python_type("string") is str

    def test_maps_integer_to_int(self):
        """Test integer type mapping."""
        from server import _get_python_type

        assert _get_python_type("integer") is int

    def test_maps_number_to_float(self):
        """Test number type mapping."""
        from server import _get_python_type

        assert _get_python_type("number") is float

    def test_maps_boolean_to_bool(self):
        """Test boolean type mapping."""
        from server import _get_python_type

        assert _get_python_type("boolean") is bool

    def test_defaults_to_str_for_unknown_types(self):
        """Test that unknown types default to str."""
        from server import _get_python_type

        assert _get_python_type("unknown") is str
        assert _get_python_type("array") is str
        assert _get_python_type("object") is str
...
class TestResourceEndpoints:
    ...
    def test_server_info_returns_json(self):
        ...
        result = server_info.fn() if hasattr(server_info, "fn") else server_info()
    ...
    def test_tools_list_returns_json(self):
        ...
        # Access the underlying function via .fn attribute
        result = tools_list.fn() if hasattr(tools_list, "fn") else tools_list()
...
class TestToolRegistration:
    ...
    def test_tool_function_execution_flow(self, mock_execute):
        ...
        schema = {
            "type": "object",
            "properties": {"drug_smiles": {"type": "string", "description": "Drug SMILES string"}},
        ...
        mock_execute.assert_called_once_with("predict_toxicity", {"drug_smiles": "CC(=O)O"})
...
class TestErrorHandling:
    ...
    def test_invalid_parameter_name_raises_value_error(self):
        ...
        schema = {
            "type": "object",
            "properties": {"invalid.param": {"type": "string"}},
            "required": [],
        }
...
class TestServerGPU:
    ...
    def test_tool_execution_with_real_model(self):
        ...
        test_args = dict.fromkeys(tool.inputSchema.get("required", []), "CC(=O)O")
