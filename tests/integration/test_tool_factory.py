...
def mock_loader():
    ...
    def mock_filter_by_placeholder(ph: str, exact: bool = True):
        return {name: tmpl for name, tmpl in templates.items() if tmpl.has_placeholder(ph)}
...
class TestParameterNormalization:
    ...
    def test_normalize_multiple_spaces(self):
        ...
        assert (
            normalize_parameter_name("Epitope amino acid sequence") == "epitope_amino_acid_sequence"
        )
...
class TestBuildTools:
    ...
    def test_filter_by_multiple_placeholders_match_any(self, mock_get_loader, mock_loader):
        ...
        tools = build_tools(filter_placeholders=["Drug SMILES", "Target sequence"], match_all=False)
    ...
    def test_combined_filters(self, mock_get_loader, mock_loader):
        ...
        tools = build_tools(
            filter_placeholder="Drug SMILES", max_placeholders=1, exclude_name_pattern="^CYP"
        )
...
class TestIntegration:
    ...
    def test_parameter_mapping_consistency(self, mock_get_loader, mock_loader):
        ...
        for tool in tools:
            if any(
                prop["title"] == "Drug SMILES" for prop in tool.inputSchema["properties"].values()
            ):
                assert "drug_smiles" in tool.inputSchema["properties"]
